...
public final class ServerListFake extends L2LoginServerPacket {
    ...
    public ServerListFake(Account account) {
        ...
        for (GameServer gs : GameServerManager.getInstance().getGameServers()) {
            ...
            _servers.add(new ServerData(gs.getId(), ip, gs.getPort(), gs.isPvp(), gs.isShowingBrackets(), gs.getServerType(), gs.getOnline(), gs.getMaxPlayers(), gs.isOnline(), entry == null ? 0 : entry.getKey(), gs.getAgeLimit(), entry == null ? new int[0] : entry.getValue()));
        }
    }
