...
import l2trunk.gameserver.network.serverpackets.AskJoinAlliance;
...
public final class RequestJoinAlly extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (obj instanceof Player && obj != activeChar) {
            ...
            if (target.getAlliance() != null || activeChar.getAlliance().isMember(target.getClan().clanId())) {
            ...
            if (!activeChar.getAlliance().canInvite()) {
                activeChar.sendMessage(new CustomMessage("l2trunk.gameserver.clientpackets.RequestJoinAlly.InvitePenalty"));
                return;
            }
            ...
            target.sendPacket(new AskJoinAlliance(activeChar.objectId(), activeChar.getName(), activeChar.getAlliance().getAllyName()));
        } else {
