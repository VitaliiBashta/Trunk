...
public class AugmentationData {
    ...
    private void load() {
        ...
        try {
            ...
            Path file = Config.DATAPACK_ROOT.resolve("data/stats/augmentation/augmentation_skillmap.xml");
        ...
        for (int i = 1; i < 5; i++) {
            try {
                ...
                Path file = Config.DATAPACK_ROOT.resolve("data/stats/augmentation/augmentation_stats" + i + ".xml");
                InputStream stream = Files.newInputStream(file);
                InputStream output;
                if ((byte) stream.read() == 0x00) {
                    byte[] bytes = new byte[0];
            ...
            try {
                ...
                Path file = Config.DATAPACK_ROOT.resolve("data/stats/augmentation/augmentation_jewel_stats" + i + ".xml");
                InputStream stream = Files.newInputStream(file);
                InputStream output;
                if ((byte) stream.read() == 0x00) {
                    byte[] bytes = new byte[0];
