package l2trunk.gameserver.vote;
...
public class DDSConverter {
    ...
    public static ByteBuffer convertToDDS(Path file) {
        if (file == null) {
        ...
        try {
            bufferedimage = ImageIO.read(Files.newInputStream(file));
        } catch (IOException e) {
    ...
    private static ByteBuffer convertToDxt1NoTransparency(BufferedImage bufferedimage) {
        if (bufferedimage == null) {
            return null;
        }
        int ai[] = new int[16];
    ...
    private static ByteBuffer convertToDxt3(BufferedImage bufferedimage) {
        if (bufferedimage == null) {
            return null;
        }
        if (!bufferedimage.getColorModel().hasAlpha()) {
    ...
    private static void buildHeaderDxt3(ByteBuffer bytebuffer, int i, int j) {
        bytebuffer.rewind();
        bytebuffer.put((byte) 68);
        bytebuffer.put((byte) 68);
        bytebuffer.put((byte) 83);
