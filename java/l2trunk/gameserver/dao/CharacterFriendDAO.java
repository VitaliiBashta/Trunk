...
public final class CharacterFriendDAO {
        ...
        try (PreparedStatement statement = con.prepareStatement("SELECT f.friend_id, c.char_name, s.class_id, s.level FROM character_friends f LEFT JOIN characters c ON f.friend_id = c.obj_Id LEFT JOIN character_subclasses s ON ( f.friend_id = s.char_obj_id AND s.active =1 ) WHERE f.char_id = ?")) {
            ...
            try (ResultSet rset = statement.executeQuery()) {
                while (rset.next()) {
                    ...
                    int level = rset.getInt("s.occupation");
    ...
    public void insert(Player owner, Player friend) {
        try (Connection con = DatabaseFactory.getInstance().getConnection();
            ...
            statement.setInt(1, owner.objectId());
            statement.setInt(2, friend.objectId());
            statement.execute();
        } catch (SQLException e) {
            _log.warn(owner.getFriendList() + " could not add friend objectid: " + friend.objectId(), e);
        }
    }
    ...
    public void delete(Player owner, int friend) {
        try (Connection con = DatabaseFactory.getInstance().getConnection();
            ...
            statement.setInt(4, owner.objectId());
            statement.execute();
        } catch (SQLException e) {
            _log.warn("FriendList: could not delete friend objectId: " + friend + " ownerId: " + owner.objectId(), e);
        }
    }
