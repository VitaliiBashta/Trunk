...
public final class AutoBan {
    ...
    public static boolean ChatBan(String actor, int period, String msg, String GM) {
        ...
        if (plyr != null) {
            ...
            plyr.sendMessage(new CustomMessage("l2trunk.Util.AutoBan.ChatBan", plyr).addString(GM).addNumber(period));
            plyr.updateNoChannel(NoChannel);
        } else
            try (Connection con = DatabaseFactory.getInstance().getConnection();
                 PreparedStatement statement = con.prepareStatement("UPDATE characters SET nochannel = ? WHERE obj_Id=?")) {
    ...
    public static boolean ChatUnBan(String actor, String GM) {
        ...
        PreparedStatement statement = null;
        if (plyr != null) {
            plyr.sendMessage(new CustomMessage("l2trunk.Util.AutoBan.ChatUnBan", plyr).addString(GM));
