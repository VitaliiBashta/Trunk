...
public class CaptchaTimer {
     ...
     */
    void addBotReporter(Player player) {
        FailedBotReporter reporter = getBotReporter(player);
        if (reporter != null)
            reporter.setLastReportTime(System.currentTimeMillis());
        else
            failedBotReporters.add(new FailedBotReporter(player.getAccountName(), player.getIP(), System.currentTimeMillis()));
    }
     ...
     */
    void removeBotReporter(Player player) {
        FailedBotReporter reporter = getBotReporter(player);
        if (reporter != null)
     ...
     */
    boolean canReportBotAgain(Player player) {
        FailedBotReporter reporter = getBotReporter(player);
     ...
     */
    void addCaptchaTimer(Player actor, Player target, String correctCaptcha) {
        target.block();
     ...
     */
    void removeCaptchaTimer(CaptchaEvent event) {
     ...
     */
    CaptchaEvent getMyEvent(Player target) {
     ...
     */
    private Iterable<CaptchaEvent> getCaptchaEventList() {
        return captchaEventList;
    }
    ...
    private static class FailedBotReporter {
        ...
        private FailedBotReporter(String accountName, String ip, long lastReportTime) {
            this.accountName = accountName;
            this.ip = ip;
            this.lastReportTime = lastReportTime;
        }

        void setLastReportTime(long lastReportTime) {
            this.lastReportTime = lastReportTime;
        }
    ...
    private static class CaptchaTimerHolder {
        static final CaptchaTimer instance = new CaptchaTimer();
    }

    class CaptchaTimerThread implements Runnable {
