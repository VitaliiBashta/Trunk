...
public final class AdminEditChar implements IAdminCommandHandler {
        ...
        if (fullString.equals("admin_current_player"))
            ...
            if (st.countTokens() > 1) {
                ...
                if (!player.addSubClass(classId, true, 0)) {
                    activeChar.sendMessage(new CustomMessage("l2trunk.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", activeChar));
                    return false;
                }
    ...
    private void setSubclass(final Player activeChar, final Player player) {
        ...
        if (subsAvailable != null && !subsAvailable.isEmpty()) {
            ...
            activeChar.sendMessage(new CustomMessage("l2trunk.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", activeChar));
            return;
        }
