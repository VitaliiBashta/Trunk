...
public final class PetitionManager implements IPetitionHandler {
    ...
    private int getPendingPetitionCount() {
    ...
    private boolean isPetitioningAllowed() {
    ...
    private class Petition {
        ...
        Petition(Player petitioner, String petitionText, int petitionType) {
            _id = getNextId();
        ...
        List<Say2> getLogMessages() {
            return _messageLog;
        }

        boolean endPetitionConsultation(PetitionState endState) {
            setState(endState);
        ...
        Player getPetitioner() {
            return World.getPlayer(_petitioner);
        }

        int getPetitionerObjId() {
            return _petitioner;
        }
        ...
        PetitionState getState() {
            return _state;
        }

        void setState(PetitionState state) {
            _state = state;
        }
        ...
        void sendPetitionerPacket(L2GameServerPacket responsePacket) {
            if ((getPetitioner() == null) || !getPetitioner().isOnline()) {
        ...
        void sendResponderPacket(L2GameServerPacket responsePacket) {
            if ((getResponder() == null) || !getResponder().isOnline()) {
