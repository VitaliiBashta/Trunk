...
import l2trunk.gameserver.model.entity.events.fightclubmanager.*;
import l2trunk.gameserver.model.entity.events.fightclubmanager.FightClubEventManager.CLASSES;
import l2trunk.gameserver.model.entity.events.fightclubmanager.FightClubLastStatsManager.FightClubStatType;
...
public abstract class AbstractFightClub extends GlobalEvent {
     ...
     */
    private static String getFixedTime(int seconds) {
    ...
    private void startTeleportTimer(FightClubGameRoom room) {
     ...
     * @param healAndRess
     */
    private void teleportSinglePlayer(FightClubPlayer fPlayer, boolean firstTime, boolean healAndRess) {
    ...
    private void teleportBackToTown(Player player) {
        if (player.isDead())
            player.doRevive();
        player.setPolyId(0);
    ...
    private void rewardPlayer(FightClubPlayer fPlayer, boolean isTopKiller) {
     ...
     */
    private void announceWinnerTeam(boolean wholeEvent, FightClubTeam winnerOfTheRound) {
        int bestScore = -1;
    ...
    private void updateScreenScores(Player player) {
    ...
    private String getScorePlayerName(FightClubPlayer fPlayer) {
    ...
    private void showScores(Creature c) {
    ...
    private void hideScores(Creature c) {
     ...
     */
    private void setInvisible(Player player, int seconds, boolean sendMessages) {
        FightClubPlayer fPlayer = getFightClubPlayer(player);
    ...
    private void rootPlayer(Player player) {
    ...
    private void ressAndHealPlayers() {
    ...
    private void sendMessageToRegistered(MESSAGE_TYPES type, String msg) {
        //Registered
        for (FightClubPlayer iFPlayer : getPlayers(REGISTERED_PLAYERS))
            sendMessageToPlayer(iFPlayer, type, msg);
    }
    ...
    protected void sendMessageToPlayer(FightClubPlayer fPlayer, MESSAGE_TYPES type, String msg) {
        sendMessageToPlayer(fPlayer.getPlayer(), type, msg);
    }

    private void sendMessageToPlayer(Player player, MESSAGE_TYPES type, String msg) {
        switch (type) {
            case GM:
    ...
    protected void setState(EVENT_STATE state) {
        _state = state;
    }
    ...
    private boolean isRootBetweenRounds() {
    ...
    private int getTeamTotalKills(FightClubTeam team) {
        if (!isTeamed())
            return 0;
        int totalKills = 0;
        for (FightClubPlayer iFPlayer : team.getPlayers())
    ...
    protected List<FightClubPlayer> getPlayers(String... groups) {
        if (groups.length == 1) {
            return getObjects(groups[0]);
        } else {
     ...
     * @return
     */
    private FightClubPlayer getFightClubPlayer(Creature creature, String... groups) {
     ...
     */
    private Location getTeamSpawn(FightClubPlayer fPlayer, boolean randomNotClosestToPt) {
    ...
    public enum PlayerComparator implements Comparator<Player> {
        LEVEL_SORT {
            ...
            public int compare(Player left, Player right) {
                ...
                return Integer.compare(left.getLevel(), right.getLevel());
            }
        },
        CLASS_SORT {
            @Override
            public int compare(Player left, Player right) {
                ...
                return Integer.compare(left.getClassId().getType2().ordinal(), right.getClassId().getType2().ordinal());
            }
        };
        ...
        static Comparator<Player> getComparator(final PlayerComparator... multipleOptions) {
            return (o1, o2) -> {
