...
public final class OlympiadDatabase {
    private static final Logger LOG = LoggerFactory.getLogger(OlympiadDatabase.class);

    public static synchronized void loadNoblesRank() {
        Olympiad.noblesRank = new ConcurrentHashMap<>();
        ...
        try (Connection con = DatabaseFactory.getInstance().getConnection();
            ...
            LOG.error("Olympiad System: Error!", e);
        }
    ...
    public static synchronized void cleanupNobles() {
        LOG.info("Olympiad: Calculating last period...");
        try (Connection con = DatabaseFactory.getInstance().getConnection()) {
            ...
            LOG.error("Olympiad System: Couldn't calculate last period!", e);
        }

        Olympiad.nobles.values().forEach(stats -> {
            int points = stats.getInteger(Olympiad.POINTS);
            int compDone = stats.getInteger(Olympiad.COMP_DONE);
            stats.set(Olympiad.POINTS, Config.OLYMPIAD_POINTS_DEFAULT);
            if (compDone >= Config.OLYMPIAD_BATTLES_FOR_REWARD) {
                stats.set(Olympiad.POINTS_PAST, points);
                stats.set(Olympiad.POINTS_PAST_STATIC, points);
            } else {
                stats.unset(Olympiad.POINTS_PAST);
                stats.unset(Olympiad.POINTS_PAST_STATIC);
            }
            stats.unset(Olympiad.COMP_DONE);
            stats.unset(Olympiad.COMP_WIN);
            stats.unset(Olympiad.COMP_LOOSE);
            stats.unset(Olympiad.GAME_CLASSES_COUNT);
            stats.unset(Olympiad.GAME_NOCLASSES_COUNT);
            stats.unset(Olympiad.GAME_TEAM_COUNT);
        });
    }
    ...
    public static List<String> getClassLeaderBoard(int classId) {
        ...
        try (Connection con = DatabaseFactory.getInstance().getConnection();
            ...
            LOG.error("Olympiad System: Couldn't get old noble ranking from db!", e);
        }
    ...
    public static List<String> getClassLeaderBoardCurrent(int classId) {
        ...
        try (Connection con = DatabaseFactory.getInstance().getConnection();
            ...
            LOG.error("Olympiad System: Couldn't get current noble ranking from db!", e);
        }
    ...
    public static synchronized void sortHerosToBe() {
        ...
        try (Connection con = DatabaseFactory.getInstance().getConnection()) {
            ...
            LOG.error("Olympiad System: Couldnt heros from db!", e);
        }
    }
    ...
    static synchronized void saveNobleData() {
        if (Olympiad.nobles == null)
            return;
        Olympiad.nobles.keySet().forEach(OlympiadDatabase::saveNobleData);
    }
