...
public final class PDam extends Skill {
    ...
    public PDam(StatsSet set) {
        super(set);
        onCrit = set.isSet("onCrit");
        directHp = set.isSet("directHp");
        turner = set.isSet("turner");
        blow = set.isSet("blow");
    }
        ...
        for (Creature target : targets)
            if (target != null && !target.isDead()) {
                ...
                if (!info.miss || info.damage >= 1)
                    realTarget.reduceCurrentHp(info.damage, activeChar, this, true, true, !info.lethal && directHp, true, false, false, power != 0);
