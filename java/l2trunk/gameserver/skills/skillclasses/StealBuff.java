...
public final class StealBuff extends Skill {
    ...
    private static boolean canBeStolen(Effect e) {
        ...
        if (e.skill.isToggle())
            return false;
        if (e.skill.isPassive())
            return false;
        if (e.skill.isOffensive)
            return false;
        ...
        if (target instanceof Player) {
            ...
            if (!effectList.isEmpty()) {
                ...
                for (Effect effect : effectList) {
                    ...
                    effect.exit();
                    target.sendPacket(new SystemMessage2(SystemMsg.THE_EFFECT_OF_S1_HAS_BEEN_REMOVED).addSkillName(effect.skill.id, effect.skill.level));
                    count++;
