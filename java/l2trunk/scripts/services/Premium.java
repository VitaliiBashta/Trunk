...
import l2trunk.gameserver.network.loginservercon.gspackets.BonusRequest;
import l2trunk.gameserver.network.serverpackets.ConfirmDlg;
import l2trunk.gameserver.network.serverpackets.ExBR_PremiumState;
...
public class Premium extends Functions {
    ...
    private class Ask implements OnAnswerListener {
        ...
        Ask(Player player, Player target, int days, long cost, int item, int rate) {
            _player = player;
            this.days = days;
            this.cost = cost;
            this.item = item;
    ...
    private void gift(Player player, int time, int rate) {
        int current = (int) (System.currentTimeMillis() / 1000L);
        int bonusExpire = player.getNetConnection().getBonusExpire();
        int newBonusTime = bonusExpire == 0 ? current + time : bonusExpire + time;
